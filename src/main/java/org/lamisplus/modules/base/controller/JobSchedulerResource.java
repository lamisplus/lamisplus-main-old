...
import org.lamisplus.modules.base.backup.vm.ScheduleResponse;
import org.lamisplus.modules.base.backup.vm.SchedulerTrigger;
import org.quartz.*;
...
public class JobSchedulerResource {
    ...
    public void removeJobsInErrorState() throws SchedulerException {
        //LOG.debug("Delete jobs in Error state");
        for (String groupName : scheduler.getJobGroupNames()) {
            for (JobKey jobKey : scheduler.getJobKeys(GroupMatcher.jobGroupEquals(groupName))) {
                List<? extends Trigger> triggers = scheduler.getTriggersOfJob(jobKey);
                triggers.forEach(trigger -> {
